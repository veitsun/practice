...
size_t CacheFile::read(char* destination , size_t size)
{
    ...
    while (totalRead < size)
    {
        ...
        std::memcpy( destination + totalRead, buffer.data() + buffer_Position ,  to_copy);
        totalRead += to_copy;
...
size_t CacheFile::write(char* data,size_t numbytes)
{
    ...
    while (totalWrite < numbytes)
    {
        ...
        std::memcpy(buffer.data() + buffer_Position, data + totalWrite, data_to_buffer);
...
std::streampos CacheFile::lseek( std::streampos offset , std::ios_base::seek_dir whence)
{
