    ...
    if (this->m_n != p.m_n) {
      ...
      for (int row = 0; row < m_n; row++) {
        for (int col = 0; col < m_n; col++) {
          for (int k = 0; k < m_n; k++) {
            // cout<<(row*m_n + col)<< "     "<<(row*m_n + k) <<"     "<<(k*m_n
            // + col)<<endl;

            // cout<<temp.m_ptr[row*m_n + col]<<"      "<<m_ptr[row*m_n + k]<<"
            // "<<p.m_ptr[k*m_n + col] <<endl;

            temp.m_ptr[row * m_n + col] +=
                (m_ptr[row * m_n + k] * p.m_ptr[k * m_n + col]);
          }
        }
      }
